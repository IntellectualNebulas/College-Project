...
# Configured to serve templates and raw asset files directly out of the WebsiteCollege directory root
app = Flask(
    __name__, 
    template_folder='../WebsiteCollege',
    static_folder='../WebsiteCollege',
    static_url_path=''  # 🛠️ FIXED: Removes the hidden 'static' prefix requirement completely
)
...
def load_game_files():
    ...
    try:
        # Dynamic base-path calculation utility
        ...
        # Assign a random killer from the file
        if game_data["suspects"]:
            ...
            for suspect in game_data["suspects"]:
                ...
                if s_name.lower() == game_data["killer_name"].lower():
                    ...
                    game_data["digital_footprints"][s_name] = {
                        "email": f"INVOICE #4112: 1x Professional uniform cleaning kit for a {suspect.get('Professions')}.",
                        "search_history": [f"trends for {suspect.get('HairColour')} hair care", "how to fix a leaking tap", "python web ui design templates"],
...
# Call file loading logic right away when Python starts up
load_game_files()
